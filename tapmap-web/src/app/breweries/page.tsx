...
export default async function BreweriesPage({ searchParams }: BreweriesPageProps) {
  ...
  return (
    <main className="flex-1 p-8 md:flex md:h-[calc(100vh-4rem)] md:flex-none md:flex-col md:overflow-hidden">
      <h1 className="text-2xl font-semibold text-foreground md:flex-none">Breweries</h1>
      <div className="mt-4 flex flex-col gap-4 md:flex-row md:flex-1 md:gap-6 md:overflow-hidden">
        <div className="md:flex md:h-full md:w-[300px] md:flex-none md:flex-col md:overflow-hidden">
          <div className="rounded-lg border border-border bg-card p-4 md:flex-none">
          ...
          {breweries.length === 0 ? (
            <p className="mt-6 text-muted-foreground md:flex-1 md:overflow-y-auto">
              {name || city ? 'No breweries match your filters.' : 'No breweries found.'}
            </p>
          ) : (
            <div className="mt-6 flex flex-col gap-4 md:flex-1 md:overflow-y-auto">
        ...
        <div className="order-first w-full md:order-last md:h-full md:flex-1">
